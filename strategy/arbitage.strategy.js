...
const getArbitagePriceByCurrencyList = exports.getArbitagePriceByCurrencyList = async function (interestedCurrency) {
...
exports.getArbitagePriceStrategy = {
  ...
  messageReducer: async (error, result) => {
    ...
    return {
      type: 'text',
      text: `bitfinex ราคา ${result.currency} แพงกว่า ${result.marginPercent.toFixed(2)}% (${result.margin.toFixed(3)} THB)`
    }
  }
}
...
exports.getArbitagePriceListStrategy = {
  ...
  resolve: async (action) => {
    const result = await getArbitagePriceByCurrencyList(['omg', 'btc', 'xrp', 'eth', 'dash'])
    return result
  },
  conditionResolve: async (error, result, notification) => {
    const { payload, condition, _id } = notification
